...
export interface CreateDocumentInput {
  ...
  // Liens relationnels directs
  clientId?: string
  projetId?: string
  tacheId?: string
  // Métadonnées de stockage (optionnelles)
...
export interface UpdateDocumentInput {
  ...
  signedAt?: Date
  signedBy?: Prisma.InputJsonValue
}
...
export class DocumentService {
  ...
  private formatDocument(
    ...
    const formatted: Record<string, unknown> & { id: string } = {
      ...document,
      id: docId,
      fileSize: this.toNumber(document.fileSize),
      uploadedBy: document.uploadedBy ? {
        id: (document.uploadedBy as RelationItem).id,
        firstName: (document.uploadedBy as RelationItem).firstName,
        lastName: (document.uploadedBy as RelationItem).lastName,
        email: (document.uploadedBy as RelationItem).email,
      } : undefined,
      clients: (document.clients as RelationItem[] | undefined)?.map((cd) => ({
        ...cd,
        client: cd.client ? {
          id: (cd.client as RelationItem).id,
          firstName: (cd.client as RelationItem).firstName,
          lastName: (cd.client as RelationItem).lastName,
        } : undefined,
      })),
      actifs: (document.actifs as RelationItem[] | undefined)?.map((ad) => ({
        ...ad,
        actif: ad.actif ? {
          id: (ad.actif as RelationItem).id,
          name: (ad.actif as RelationItem).name,
          type: (ad.actif as RelationItem).type,
        } : undefined,
      })),
      passifs: (document.passifs as RelationItem[] | undefined)?.map((pd) => ({
        ...pd,
        passif: pd.passif ? {
          id: (pd.passif as RelationItem).id,
          name: (pd.passif as RelationItem).name,
          type: (pd.passif as RelationItem).type,
        } : undefined,
      })),
      contrats: (document.contrats as RelationItem[] | undefined)?.map((cd) => ({
        ...cd,
        contrat: cd.contrat ? {
          id: (cd.contrat as RelationItem).id,
          name: (cd.contrat as RelationItem).name,
          type: (cd.contrat as RelationItem).type,
        } : undefined,
      })),
      projets: (document.projets as RelationItem[] | undefined)?.map((pd) => ({
        ...pd,
        projet: pd.projet ? {
          id: (pd.projet as RelationItem).id,
          name: (pd.projet as RelationItem).name,
        } : undefined,
      })),
      taches: (document.taches as RelationItem[] | undefined)?.map((td) => ({
        ...td,
        tache: td.tache ? {
          id: (td.tache as RelationItem).id,
          title: (td.tache as RelationItem).title,
        } : undefined,
      })),
    }
  ...
  async listDocuments(filters?: {
    ...
    const where: Prisma.DocumentWhereInput = { cabinetId: this.cabinetId }
    ...
    if (filters?.uploadedAfter || filters?.uploadedBefore) {
      where.uploadedAt = {
        ...(filters.uploadedAfter ? { gte: filters.uploadedAfter } : {}),
        ...(filters.uploadedBefore ? { lte: filters.uploadedBefore } : {}),
      }
    }
  ...
  async updateDocument(id: string, data: UpdateDocumentInput) {
    ...
    if (data.signatureStatus === 'SIGNE' && existingDoc.signatureStatus !== 'SIGNE') {
  ...
  async getDocumentVersions(documentId: string) {
    ...
    if (document.parentVersionId) {
      // Remonter jusqu'à la racine
      let current = document
      while (current.parentVersionId) {
        const parent = await this.prisma.document.findFirst({
          where: {
            id: current.parentVersionId,
            cabinetId: this.cabinetId,
          },
        })
        if (!parent) break
    ...
    const allVersions = await this.prisma.document.findMany({
      where: {
        OR: [
          ...
          { parentVersionId: rootId, cabinetId: this.cabinetId },
        ],
      },
      include: {
        uploadedBy: {
  ...
  async searchByTags(tags: string[]) {
    ...
    const documents = allDocuments.filter(doc => {
      ...
      const docTags = doc.tags as string[] | null
  ...
  async getDocumentStats() {
    ...
    const byType = documents.reduce<Record<string, number>>((acc, d: DocumentRecord) => {
      const type = d.type
      if (!acc[type]) {
        acc[type] = 0
      }
    ...
    const byCategory = documents.reduce<Record<string, number>>((acc, d: DocumentRecord) => {
