...
export class PatrimoineService {
  ...
  private formatClientActif(clientActif: Record<string, unknown> | null): Record<string, unknown> | null {
    if (!clientActif) {
      return null
    }
    ...
    if (clientActif.actif) {
      formatted.actif = this.formatActif(clientActif.actif as Record<string, unknown>)
    }

    return formatted
  }

  private formatActif(actif: Record<string, unknown> | null): Record<string, unknown> | null {
    if (!actif) {
      return null
    }
    ...
    return {
      ...
      clients: (actif.clients as Record<string, unknown>[] | undefined)?.map((clientActif) => this.formatClientActif(clientActif)) ?? [],
    }
  }

  private formatPassif(passif: Record<string, unknown> | null): Record<string, unknown> | null {
    if (!passif) {
      return null
    }
  ...
  private formatContrat(contrat: Record<string, unknown> | null): Record<string, unknown> | null {
    if (!contrat) {
      return null
    }
  ...
  async listActifs(filters?: {
    ...
    if (filters?.minValue !== undefined || filters?.maxValue !== undefined) {
      where.value = {
        ...(filters?.minValue !== undefined ? { gte: new Decimal(filters.minValue) } : {}),
        ...(filters?.maxValue !== undefined ? { lte: new Decimal(filters.maxValue) } : {}),
      }
    }
  ...
  async updatePassif(id: string, data: Partial<CreatePassifInput>) {
    ...
    // Recalculer le patrimoine du client
    const clientId =
      passif && typeof passif === 'object' && !Array.isArray(passif) &&
      'clientId' in passif && typeof (passif as { clientId?: unknown }).clientId === 'string'
        ? (passif as { clientId: string }).clientId
        : null
  ...
  async getCabinetStats() {
    ...
    const contratsActifs = contrats.filter(contrat => contrat.status === 'ACTIF').length
