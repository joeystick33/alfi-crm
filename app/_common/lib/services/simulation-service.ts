...
import { SimulationStatus, type SimulationType } from '@prisma/client'
...
export class SimulationService {
  ...
  private formatSimulation(simulation: any): Record<string, unknown> | null {
    if (!simulation) {
      return null
    }
  ...
  async getStatistics() {
    ...
    const [total, completed, shared, archived] = await Promise.all([
      ...
      this.prisma.simulation.count({ where: { ...where, status: 'TERMINE' } }),
      this.prisma.simulation.count({ where: { ...where, sharedWithClient: true } }),
      this.prisma.simulation.count({ where: { ...where, status: 'ARCHIVE' } }),
    ])
