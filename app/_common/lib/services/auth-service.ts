...
export class AuthService {
  ...
  static async loginUser(
    ...
    // Vérifier que le cabinet est actif (accepte ACTIVE ou ACTIF pour compatibilité)
    const activeStatuses = ['ACTIVE', 'ACTIF', 'TRIALING']
    if (!activeStatuses.includes(user.cabinet.status)) {
  ...
  /**
   * Authentifie un client (portail client)
   */
  static async loginClient(
    ...
    // Vérifier le mot de passe
    const isValidPassword = await bcrypt.compare(password, client.portalPassword)
    if (!isValidPassword) {
      return null
    }

    // Vérifier que le client est actif
    if (client.status !== 'ACTIF' && client.status !== 'PROSPECT') {
      throw new Error('Client account is not active')
    }

    // Mettre à jour la date de dernière connexion portail
