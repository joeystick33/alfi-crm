 ...
 */
export async function validateQuotas(cabinetId: string, newQuotas: { maxAdvisors?: number; maxAssistants?: number }) {
...
export async function checkQuotaLimit(
  ...
  const org = await getOrganizationWithUsage(cabinetId);
  const usage = (org.usage as Record<string, number>) || {};
  const quotas = (org.quotas as Record<string, number>) || {};
...
export async function getOrganizationsWithQuotaAlerts() {
  ...
  const alerts: Array<{ cabinetId: string; organizationName: string; quotaType: string; current: number; max: number; percentage: number; alertLevel: string }> = [];
