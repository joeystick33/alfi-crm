...
export default function OpportunitesDetecteesPage() {
  ...
  const loadOpportunities = async (currentFilters = filters, signal?: AbortSignal) => {
    try {
      ...
      const response = await api.get('/opportunites', { params, signal } as any)
      ...
      setError(error.message || 'Erreur lors du chargement des opportunités')
    } finally {
      setLoading(false)
    }
  }
  ...
  const resumeStats = useMemo(() => {
    if (!stats) {
      ...
      const totalPotential = opportunities.reduce((sum: any, o: any) => sum + (o.estimatedValue || o.potentialGain || 0), 0)
      const averageScore = opportunities.length > 0
        ? opportunities.reduce((sum: any, o: any) => sum + (o.score || 0), 0) / opportunities.length
        : 0
      const highPriority = opportunities.filter((o: any) => o.priority === 'HIGH' || o.priority === 'URGENT').length
                  ...
                  >
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {opportunity.status && (
                              <Badge variant="outline" className="text-xs">
                                {opportunity.status === 'DETECTED' ? 'À analyser' : opportunity.status}
                              </Badge>
                            )}
                          </div>
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <div className="rounded-xl border border-blue-100 bg-blue-50/80 px-3 py-2 text-xs text-blue-700 dark:border-blue-900 dark:bg-blue-950/50">
