...
export function normalizeOpportuniteCreatePayload(body: unknown): CreateOpportunitePayload {
  ...
  const estimatedValue = ensureNumber(data.estimatedValue, 'estimatedValue')
  const probability = ensureNumber(data.probability, 'probability')
  
  const expectedCloseDate = ensureDate(data.expectedCloseDate, 'expectedCloseDate')
...
export function normalizeOpportuniteUpdatePayload(body: unknown): UpdateOpportunitePayload {
  ...
  const rejectionReason = ensureString(data.rejectionReason, 'rejectionReason')
  if (rejectionReason !== undefined) update.rejectionReason = rejectionReason

  const convertedToProjetId = ensureString(data.convertedToProjetId, 'convertedToProjetId')
