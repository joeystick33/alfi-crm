...
export function normalizeProjetCreatePayload(body: unknown): CreateProjetPayload {
  ...
  const estimatedBudget = ensureNumber(data.estimatedBudget, 'estimatedBudget')
  const actualBudget = ensureNumber(data.actualBudget, 'actualBudget')
  const progress = ensureNumber(data.progress, 'progress')
