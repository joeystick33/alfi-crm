...
export async function GET(
  ...
  try {
    ...
    const kycCheck = await service.checkClientKYC(clientId)

    return createSuccessResponse(kycCheck)
  } catch (error: any) {
    logger.error('Error in GET /api/clients/[id]/kyc:', { error: error instanceof Error ? error.message : String(error) })
...
export async function POST(
  ...
  try {
    ...
    logger.error('Error in POST /api/clients/[id]/kyc:', { error: error instanceof Error ? error.message : String(error) })
