...
const simulationTypeIcons: Record<string, React.ElementType> = {
...
export function SimulationHistory({ clientId }: SimulationHistoryProps) {
  ...
  const loadSimulations = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await apiCall<{ data?: SimulationWithDetails[] } | SimulationWithDetails[]>(
        `/advisor/simulations?clientId=${clientId}`
      )
      // Handle API response wrapper { data: [...], timestamp }
  ...
  const handleShare = async (simulationId: string) => {
    try {
      await apiCall(`/advisor/simulations/${simulationId}/share`, {
        method: 'POST',
      })
      ...
      toast({ title: 'Erreur', description: 'Erreur lors du partage de la simulation', variant: 'destructive' })
    }
  }

  const handleArchive = async (simulationId: string) => {
    try {
      await apiCall(`/advisor/simulations/${simulationId}`, {
        method: 'PATCH',
      ...
      toast({ title: 'Erreur', description: 'Erreur lors de l\'archivage de la simulation', variant: 'destructive' })
    }
  }
