...
import { useClientTimeline } from '@/app/(advisor)/(frontend)/hooks/use-timeline'
...
const eventTypeConfig = {
  ...
  CONTRACT_SIGNED: { label: 'Contrat signé', icon: FileText, color: 'bg-[#7373FF]/100' },
...
export function TabTimeline({ clientId, client: _client }: TabTimelineProps) {
  ...
  const filteredEvents = selectedType
    ? events?.filter((event: { type: string }) => event.type === selectedType)
    : events
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            {Object.entries(eventTypeConfig).map(([type, config]) => {
              const count = events?.filter((e: { type: string }) => e.type === type).length || 0
        ...
        <CardContent>
          {filteredEvents && filteredEvents.length > 0 ? (
            <div className="relative">
              ...
              {/* Events */}
              <div className="space-y-6">
                {filteredEvents.map((event: { id: string; type: string; title?: string; description?: string; createdAt?: string; relatedEntityType?: string; relatedEntityId?: string; createdBy?: string }) => {
