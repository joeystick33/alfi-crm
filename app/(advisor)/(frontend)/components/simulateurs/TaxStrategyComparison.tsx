...
export function TaxStrategyComparison() {
  ...
  const [result, setResult] = useState<{ strategies?: { strategyName: string; investment: number; taxSavings: number; netBenefit: number; roi: number; advantages?: string[]; constraints?: string[]; description?: string; implementation?: string[] }[]; bestStrategy?: string; maxSavings?: number; recommendation?: string; baselineTax?: number } | null>(null);
  ...
  const compareStrategies = async () => {
    ...
    try {
      const response = await fetch('/api/advisor/simulators/tax/compare', {
        method: 'POST',
      ...
      setError(err instanceof Error ? err.message : 'Erreur lors de la comparaison des stratégies');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const savingsChartData = result?.strategies ? result.strategies.map((s) => ({
  ...
  const roiChartData = result?.strategies ? result.strategies.map((s) => ({
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Input
                  label="Budget disponible pour optimisation (€)"
                  type="number"
                  value={availableBudget}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAvailableBudget(e.target.value)}
                  placeholder="10000"
                  min={0}
                  required
                />
                ...
                {result.strategies.length > 0 ? (
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {result.strategies.map((strategy, index: number) => {
