...
type RetirementScenarioResult = {
  ...
  capitalAtLifeExpectancy: number
  capitalLasts: number
...
type ScenarioComparisonResult = {
  ...
  bestScenario?: (RetirementScenarioResult & { reason?: string })
  summary?: string
...
export function RetirementComparison() {
  ...
  const compareScenarios = async () => {
    ...
    try {
      const response = await fetch('/api/advisor/simulators/retirement/compare', {
        method: 'POST',
      ...
      setError(err instanceof Error ? err.message : 'Erreur lors de la comparaison des scénarios');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const savingsComparisonData: Array<{ [key: string]: string | number; name: string }> = result
  ...
  const incomeComparisonData: Array<{ [key: string]: string | number; name: string }> = result
    ? result.scenarios.map((s) => ({
        name: s.name,
              ...
              <div className="space-y-6 mt-8">
                <TimelineTemplate
                  ...
                  kpis={result.bestScenario ? [
                    ...
                    },
                    {
                      ...
                      value: `${result.scenarios.filter((s) => s.isRetirementFeasible).length}/${result.scenarios.length}`,
                      description: 'Objectifs atteignables',
                      icon: <Target className="h-5 w-5" />,
                    },
                  ] : []}
                  feasibility={{
                    status: result.scenarios.some((s) => s.isRetirementFeasible) ? 'FEASIBLE' : 'NOT_FEASIBLE',
                ...
                <div className="overflow-x-auto">
                  ...
                  <table className="w-full border-collapse">
                    ...
                    <tbody>
                      {result.scenarios.map((scenario, index: number) => (
