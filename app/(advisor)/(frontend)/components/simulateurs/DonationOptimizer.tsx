...
import { TimelineTemplate } from '@/app/_common/components/ui/bento/TimelineTemplate';
...
export function DonationOptimizer() {
  ...
  const optimizeDonations = async () => {
    ...
    try {
      const response = await fetch('/api/advisor/simulators/succession/donations', {
        method: 'POST',
      ...
      if (!response.ok || payload?.success === false) {
        const message = payload?.error || payload?.message || 'Erreur lors de l\'optimisation';
        throw new Error(message);
      }

      setResult(payload.optimization || payload.data || payload.result);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Erreur lors de l\'optimisation des donations');
      console.error(err);
    } finally {
      setLoading(false);
    }
            ...
            <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Input
                  ...
                  type="number"
                  value={donorAge}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDonorAge(e.target.value)}
                  placeholder="60"
                />
                ...
                <Input
                  ...
                  type="number"
                  value={totalWealth}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTotalWealth(e.target.value)}
                  placeholder="1000000"
                  min={0}
                  required
                />
                ...
                <Input
                  ...
                  type="number"
                  value={targetAge}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTargetAge(e.target.value)}
                  placeholder="80"
                />
              </div>
            </div>
                      ...
                      <div className="flex items-start gap-3">
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-3">
                          <Input
                            ...
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateBeneficiary(beneficiary.id, 'name', e.target.value)}
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => updateBeneficiary(beneficiary.id, 'relationship', e.target.value)}
                          ...
                          <Input
                            ...
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateBeneficiary(beneficiary.id, 'share', e.target.value)}
                            placeholder="50"
                          />
                          ...
                          <Input
                            ...
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateBeneficiary(beneficiary.id, 'previousDonations', e.target.value)}
                            placeholder="0"
                          />
                        </div>
              ...
              <div className="mt-8">
                ...
                {result.beneficiaryBreakdown && result.beneficiaryBreakdown.length > 0 && (
                  <div className="mt-8">
                    ...
                    <ModernBarChart
                      data={result.beneficiaryBreakdown.map((b) => ({
                        name: b.name,
