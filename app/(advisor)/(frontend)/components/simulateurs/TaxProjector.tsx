...
import { ModernDonutChart } from '@/app/_common/components/charts/ModernDonutChart';
import {
  TrendingUp,
  Info,
  Percent,
...
export function TaxProjector() {
  ...
  const projectTaxes = async () => {
    ...
    try {
      const response = await fetch('/api/advisor/simulators/tax/project', {
        method: 'POST',
      ...
      if (!response.ok || payload?.success === false) {
        const message = payload?.error || payload?.message || 'Erreur lors de la projection';
        throw new Error(message);
      }

      setResult(payload.data || payload.result || null);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erreur lors de la projection fiscale');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const timelineData = result?.projections ? result.projections.map((p) => ({
    year: p.year,
    age: p.age,
  ...
  const taxBreakdownData = result?.projections ? result.projections.map((p) => ({
    year: p.year,
  ...
  const effectiveRateData = result?.projections ? result.projections.map((p) => ({
    year: p.year,
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Input
                  label="Croissance annuelle (%/an)"
                  type="number"
                  value={incomeGrowthRate}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setIncomeGrowthRate(e.target.value)}
                  placeholder="2"
                  step={0.1}
                  min={-10}
                  max={20}
                ...
                <Input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setCurrentDeductions(e.target.value)}
                  placeholder="0"
                  min={0}
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                <Input
                  label="Années de projection"
                  type="number"
                  value={yearsToProject}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setYearsToProject(e.target.value)}
                  placeholder="10"
                  min={1}
                  max={50}
                  required
                ...
                <Input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFamilyQuotient(e.target.value)}
                  placeholder="1"
                  step={0.5}
                  min={1}
                  max={10}
                ...
                <div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full border-collapse text-sm">
                      ...
                      <tbody>
                        {result.projections.map((projection, index: number) => (
                          <tr key={index} className="bg-white hover:bg-gray-50">
                            <td className="p-3 border font-semibold">{projection.year}</td>
                            {projection.age !== undefined && projection.age !== null && (
      ...
      <Card className="border shadow-md">
        <CardHeader>
          <CardTitle className="text-sm font-semibold text-slate-700 uppercase tracking-widest">Répartition Fiscale (Année 1)</CardTitle>
        </CardHeader>
        <CardContent className="h-[300px]">
          <ModernDonutChart
            data={[
              { name: "Impôt sur le Revenu", value: result ? Number(result.taxAmount) : 3000, fill: "#EF4444" }, // Red
              { name: "Prélèvements Sociaux", value: result ? Number(result.socialContributions) : 1500, fill: "#F59E0B" }, // Amber
              { name: " Reste à Vivre", value: result ? (Number(currentIncome) - Number(result.totalTax)) : 45000, fill: "#10B981" } // Emerald
            ]}
            formatValue={(val) => new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(val)}
