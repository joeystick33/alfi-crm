...
export default function DocumentsWidget({ maxDocuments = 5, showStats = true }: DocumentsWidgetProps) {
  ...
  const [documents, setDocuments] = useState<{ id: string; name: string; type: string; status: string; priority?: string; clientName?: string; updatedAt?: string }[]>([])
  ...
  const loadDocuments = async () => {
    try {
      const response = await fetch(`/api/advisor/widgets/documents?limit=${maxDocuments}`)
  ...
  const getPriorityConfig = (priority: string) => {
    const configs: Record<string, { color: string; badge: string }> = {
      'HAUTE': {
  ...
  const uploadFiles = async (files: FileList) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const response = await fetch('/api/advisor/documents/upload', {
          method: 'POST',
          body: formData,
        })
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button
                        ...
                        onClick={(e: any) => {
                          ...
                          if (doc.url) {
                            ...
                            window.open(`/api/advisor/documents/${doc.id}/preview`, '_blank')
                          }
                        }}
                      >
                      ...
                      <button
                        ...
                        onClick={async (e: any) => {
                          e.stopPropagation()
                          try {
                            const response = await fetch(`/api/advisor/documents/${doc.id}/download`)
