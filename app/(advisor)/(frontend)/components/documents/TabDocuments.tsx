...
function _formatDate(value: string | Date | null | undefined) {
  if (!value) return '—'
...
function resolveDocumentUrl(doc: DocumentItem | null) {
...
export default function TabDocuments({ clientId, client }: TabDocumentsProps) {
  ...
  const [_activeTab, _setActiveTab] = useState('ENTREE_RELATION')
  const [dragActive, setDragActive] = useState(false)
  const [externalSources, setExternalSources] = useState<ExternalSourcesData | null>(null)
  const [gedLibrary, setGedLibrary] = useState<GedDocument[]>([])
  const [_gedLoading, _setGedLoading] = useState(false)
  const [showGedModal, setShowGedModal] = useState(false)
  const [gedFilterType, setGedFilterType] = useState<string | null>(null)
  const [_linkingId, setLinkingId] = useState<string | null>(null)
  const [_unlinkingId, setUnlinkingId] = useState<string | null>(null)
  ...
  const _allDocumentTypes = useMemo(() => getAllDocumentTypes(), [])
  ...
  const _handleLinkGed = useCallback(
  ...
  const _handleUnlinkGed = useCallback(
  ...
  const _filteredGedDocs = useMemo(() => {
  ...
  const _categoryStats = useMemo(() => {
    return Object.entries(DOCUMENT_CATEGORIES).reduce<Record<string, { present: number; total: number; requiredMissing: number }>>((acc, [key, category]) => {
  ...
  const _handleDelete = async (doc: DocumentItem) => {
    if (!doc?.id) return
  ...
  const _handleDownload = (doc: DocumentItem) => {
    const url = resolveDocumentUrl(doc)
    if (!url) {
      toast({
    ...
    return (
      <div className="flex flex-wrap gap-2">
        {completeness.missingDocs.slice(0, 6).map((doc, index) => {
          const badgeKey = `${doc.type}-${doc.label}-${index}`
          return (
        ...
        <Alert className="border-amber-200 bg-amber-50">
          ...
          <AlertDescription className="space-y-2 text-amber-800">
            ...
            <ul className="grid gap-2 sm:grid-cols-2">
              {completeness.expiringDocs.map((doc, index) => (
                <li key={`${doc.type}-${index}`} className="flex items-center justify-between rounded-md bg-white/70 px-3 py-2 text-xs">
      ...
      {completeness.missingDocs.length > 0 && (
        <Alert className="border-rose-200 bg-rose-50">
          ...
          <AlertDescription className="space-y-2 text-rose-800">
            ...
            <ul className="grid gap-2 sm:grid-cols-2">
              {completeness.missingDocs.map((doc, index) => (
                <li key={`${doc.type}-${index}`} className="rounded-md bg-white/70 px-3 py-2 text-xs">
                  <span className="font-medium text-slate-700">{doc.label}</span>
          ...
          <form onSubmit={handleUpload} className="space-y-4">
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="max-h-80">
                  {(getAllDocumentTypes() as DocTypeOption[]).map((type) => (
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event: React.ChangeEvent<HTMLInputElement>) => setUploadForm((prev) => ({ ...prev, name: event.target.value }))}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(event: React.ChangeEvent<HTMLInputElement>) => setUploadForm((prev) => ({ ...prev, description: event.target.value }))}
