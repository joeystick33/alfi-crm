...
export default function AdvisorRankingList() {
  ...
  const loadAdvisors = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = (await apiCall('/api/management/team/advisors')) as
        | { data?: AdvisorItem[] }
        | AdvisorItem[]
      setAdvisors(
        Array.isArray(response)
          ? response
          : (response as { data?: AdvisorItem[] }).data || []
      )
    } catch {
      ...
      <Card className="border-slate-200">
        <CardContent className="p-6">
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-slate-600">Aucun conseiller à afficher</p>
          </div>
        </CardContent>
      </Card>
    )
      ...
      <CardContent className="p-6">
        ...
        <div className="space-y-3">
          {advisors.map((advisor) => {
            ...
            const hasUrgentAlerts = (advisor.kpi?.alerts?.urgent || 0) > 0
                ...
                {hasUrgentAlerts && (
                  <div className="mt-3 flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg">
                    ...
                    <span className="text-xs font-medium text-red-700">
                      {advisor.kpi!.alerts!.urgent} alerte
                      {advisor.kpi!.alerts!.urgent > 1 ? 's' : ''} urgente
                      {advisor.kpi!.alerts!.urgent > 1 ? 's' : ''}
                    </span>
                  </div>
                )}
              </div>
