...
import { Gift, Cake, Award, CalendarHeart, ChevronRight, type LucideIcon } from 'lucide-react'
...
function getIconAndColors(type: string): IconConfig {
  switch (type) {
    case 'birthday':
...
export default function CelebrationsWidget() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setError(null)
        const data = await apiCall('/api/advisor/celebrations?rangeDays=30') as any
        ...
        if (!cancelled) {
          setError(e instanceof Error ? e.message : 'Impossible de charger les célébrations')
          setEvents([])
        }
      } finally {
