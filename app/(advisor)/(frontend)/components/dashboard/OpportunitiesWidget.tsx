...
interface Opportunity {
  ...
  particulierId?: { firstName?: string; lastName?: string }
}
...
const STATUS_CONFIGS: Record<string, StatusConfig> = {
  'detected': { label: 'Détectée', color: 'bg-blue-100 text-blue-700', icon: Lightbulb },
  'qualified': { label: 'Qualifiée', color: 'bg-purple-100 text-purple-700', icon: Target },
  'contacted': { label: 'Contactée', color: 'bg-indigo-100 text-indigo-700', icon: Calendar },
  'presented': { label: 'Présentée', color: 'bg-orange-100 text-orange-700', icon: TrendingUp },
...
const PRIORITY_CONFIGS: Record<string, PriorityConfig> = {
  'urgent': { color: 'border-red-300 bg-red-50', badge: 'destructive', label: 'Urgent' },
  'high': { color: 'border-orange-300 bg-orange-50', badge: 'warning', label: 'Haute' },
  'medium': { color: 'border-amber-300 bg-amber-50', badge: 'default', label: 'Moyenne' },
  'low': { color: 'border-slate-300 bg-slate-50', badge: 'secondary', label: 'Basse' },
}
...
function formatDeadline(deadline: string | undefined): DeadlineInfo | null {
...
export default function OpportunitiesWidget({ maxItems = 5 }: OpportunitiesWidgetProps) {
  const router = useRouter()
  const presentationMode = usePresentationMode()
  const [opportunities, setOpportunities] = useState<Opportunity[]>([])
  const [totalPipelineValue, setTotalPipelineValue] = useState(0)
  const [stats, setStats] = useState({ active: 0, requiresAction: 0, avgScore: 0 })
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const loadOpportunities = async () => {
      try {
        const data = await apiCall(`/api/advisor/opportunities?status=active&limit=${maxItems}`) as any
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="text-center py-8" role="status">
            ...
            <p className="text-sm text-slate-600 font-medium">Aucune opportunité active</p>
            <p className="text-xs text-slate-500 mt-1">Les nouvelles opportunités apparaîtront ici</p>
          </div>
        ) : (
          <div className="space-y-3" role="list" aria-labelledby="opportunities-widget-title">
            {opportunities.map((opp) => {
              ...
              const priorityConfig = PRIORITY_CONFIGS[opp.priority?.toLowerCase()] || PRIORITY_CONFIGS.medium
              const statusConfig = STATUS_CONFIGS[opp.status?.toLowerCase()] || STATUS_CONFIGS.detected
              const StatusIcon = statusConfig.icon
              const deadline = formatDeadline(opp.actionDeadline)
              const progress = STATUS_PROGRESS[opp.status?.toLowerCase()] || 0
              const requiresAction = opp.actionDeadline && new Date(opp.actionDeadline) <= new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)
              const oppAriaLabel = `Opportunité ${TYPE_LABELS[opp.type] || opp.type} pour ${clientName}, valeur estimée ${presentationMode ? 'données masquées' : formatCurrency(opp.estimatedValue)}, statut ${statusConfig.label}, progression ${progress}%${requiresAction ? ', action requise' : ''}`
              ...
              return (
                ...
                >
                  ...
                  {/* Title and Client */}
                  <p className="text-sm font-semibold text-slate-900 mb-1">{opp.title}</p>
                  <p className="text-xs text-slate-600 mb-2">{clientName}</p>
