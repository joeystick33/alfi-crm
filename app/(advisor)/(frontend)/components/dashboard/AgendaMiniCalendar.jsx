...
function getMonthMatrix(currentDate) {
  ...
  const firstDay = (first.getDay() + 6) % 7 // Lundi = 0
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const data = await apiCall(`/api/advisor/events?${params.toString()}`)
        const list = (data && (data.events || data.data?.events)) || []
        if (!cancelled) {
          setEvents(Array.isArray(list) ? list : [])
        }
      } catch (e) {
        console.error('Erreur chargement événements calendrier:', e)
        if (!cancelled) {
