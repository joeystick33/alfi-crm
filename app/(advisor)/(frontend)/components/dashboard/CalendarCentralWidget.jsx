...
import {
  ...
  TooltipContent,
  TooltipProvider
} from '@/app/_common/components/ui/Tooltip';
...
export default function CalendarCentralWidget({
  ...
  const [view, setView] = useState(controlledView || 'week');
  ...
  const buildAppointmentQuery = useCallback(() => {
  ...
  const loadEvents = useCallback(async () => {
    try {
      ...
      const data = await apiCall(`/api/advisor/appointments?${query}`);
      ...
      const formattedEvents = list
        .filter((apt) => apt?.startTime || apt?.startDate)
        .map((apt) => {
          const start = new Date(apt.startTime || apt.startDate);
          const end = new Date(apt.endTime || apt.endDate || start);
          const clientName = apt.clientName || [apt.client?.firstName, apt.client?.lastName].filter(Boolean).join(' ');
          return {
            id: apt.id,
            title: apt.title || clientName || 'Rendez-vous',
            start,
            end,
            type: apt.type,
            status: apt.status,
