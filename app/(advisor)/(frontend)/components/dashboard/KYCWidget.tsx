...
export default function KYCWidget({ presentationMode = false }: KYCWidgetProps) {
  const [stats, setStats] = useState<KYCStats | null>(null)
  const [expiring, setExpiring] = useState<KYCExpiringItem[]>([])
  ...
  const loadKYCData = async () => {
    try {
      ...
      const [statsData, expiringData] = await Promise.all([
        apiCall('/api/advisor/kyc?stats=true') as Promise<KYCStats>,
        apiCall('/api/advisor/kyc?expiring=true') as Promise<{ kyc: KYCExpiringItem[] }>,
      ])
      setStats(statsData)
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-semibold text-cobalt">{complianceRate}%</span>
          </div>
