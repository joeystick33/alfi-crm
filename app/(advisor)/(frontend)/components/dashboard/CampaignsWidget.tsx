...
type TypeConfig = { label: string; icon: typeof Mail; color: string }
type StatusConfig = { label: string; color: string; icon: typeof Clock }
...
const TYPE_CONFIGS: Record<string, TypeConfig> = {
  ...
  PHONE: { label: 'Téléphone', icon: Phone, color: 'text-purple-600 bg-purple-100' },
  MULTI_CANAL: { label: 'Multi-canal', icon: Megaphone, color: 'text-orange-600 bg-orange-100' },
}

const STATUS_CONFIGS: Record<string, StatusConfig> = {
  BROUILLON: { label: 'Brouillon', color: 'bg-gray-100 text-gray-700', icon: Clock },
  PROGRAMMEE: { label: 'Programmée', color: 'bg-blue-100 text-blue-700', icon: Clock },
  EN_COURS: { label: 'En cours', color: 'bg-green-100 text-green-700', icon: Play },
  TERMINEE: { label: 'Terminée', color: 'bg-slate-100 text-slate-700', icon: CheckCircle },
  PAUSE: { label: 'En pause', color: 'bg-amber-100 text-amber-700', icon: Pause },
  ANNULEE: { label: 'Annulée', color: 'bg-red-100 text-red-700', icon: AlertCircle },
}

export default function CampaignsWidget({ maxCampaigns = 3 }: CampaignsWidgetProps) {
  ...
  const loadCampaigns = async () => {
    try {
      const data = (await apiCall(
        `/api/advisor/campaigns?status=EN_COURS,PROGRAMMEE&limit=${maxCampaigns}`
      )) as { campaigns?: CampaignItem[] }
  ...
  const getTypeConfig = (type: string): TypeConfig => TYPE_CONFIGS[type] || TYPE_CONFIGS.EMAIL
  const getStatusConfig = (status: string): StatusConfig =>
    STATUS_CONFIGS[status] || STATUS_CONFIGS.BROUILLON

  const calculateProgress = (campaign: CampaignItem) => {
      ...
      <CardContent>
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-slate-600 font-medium">Aucune campagne active</p>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {scheduledDate && campaign.status === 'PROGRAMMEE' && (
                            <span className="text-xs text-slate-600">{scheduledDate}</span>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 p-2 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center gap-2 text-xs text-blue-700">
                        ...
                        <span className="font-medium">
                          {campaign.nextAction.count} action
                          {campaign.nextAction.count > 1 ? 's' : ''} à venir
                        </span>
