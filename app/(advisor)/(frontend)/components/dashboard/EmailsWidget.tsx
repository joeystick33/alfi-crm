...
function formatEmailTime(dateString: string | undefined): string {
...
export default function EmailsWidget() {
  ...
  useEffect(() => {
    ...
    async function loadEmails() {
      try {
        setError(null)
        const data = await apiCall('/api/advisor/emails?limit=30&order=desc') as any
        ...
        if (!cancelled) {
          setError(e instanceof Error ? e.message : 'Impossible de charger les emails')
          setEmails([])
        }
      } finally {
  ...
  const renderList = (list: EmailItem[]) => {
    if (loading) {
      return (
