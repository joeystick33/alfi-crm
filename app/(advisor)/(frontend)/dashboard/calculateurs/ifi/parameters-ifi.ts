/**
 * ══════════════════════════════════════════════════════════════════════════════
 * PARAMÈTRES FISCAUX IFI — Calculateur Impôt sur la Fortune Immobilière
 * ══════════════════════════════════════════════════════════════════════════════
 * 
...
// ══════════════════════════════════════════════════════════════════════════════
// SEUIL D'IMPOSITION - CGI art. 964 — Source : RULES.ifi
// ══════════════════════════════════════════════════════════════════════════════
export const SEUIL_IFI_2025 = {
  SEUIL_IMPOSITION: RULES.ifi.seuil_assujettissement,
  SEUIL_DECLARATION: RULES.ifi.seuil_assujettissement,
  
  INFO: 'L\'IFI est dû si le patrimoine immobilier net taxable dépasse 1 300 000 € au 1er janvier',
} as const

// ══════════════════════════════════════════════════════════════════════════════
// BARÈME IFI 2025 - CGI art. 977 — Source : RULES.ifi.bareme
// ══════════════════════════════════════════════════════════════════════════════
export const BAREME_IFI_2025 = RULES.ifi.bareme.map(t => ({
  min: t.min,
  max: t.max,
  taux: t.taux * 100,
  label: t.taux === 0 ? 'Non taxable' : `Tranche ${(t.taux * 100).toFixed(2)}%`,
}))

// ══════════════════════════════════════════════════════════════════════════════
// DÉCOTE IFI - CGI art. 977 — Source : RULES.ifi.decote
// ══════════════════════════════════════════════════════════════════════════════
export const DECOTE_IFI_2025 = {
  SEUIL_MIN: RULES.ifi.seuil_assujettissement,
  SEUIL_MAX: RULES.ifi.decote.seuil,
  COEFFICIENT: RULES.ifi.decote.taux * 100,
  MONTANT_FIXE: RULES.ifi.decote.base,
  
  // Formule : Décote = base - (taux × Patrimoine net taxable)
  // S'applique uniquement si patrimoine entre seuil_assujettissement et decote.seuil
  calculer: (patrimoineNet: number): number => {
    if (patrimoineNet < RULES.ifi.seuil_assujettissement || patrimoineNet >= RULES.ifi.decote.seuil) return 0
    return Math.max(0, RULES.ifi.decote.base - patrimoineNet * RULES.ifi.decote.taux)
  },
...
export const ABATTEMENTS_IFI_2025 = {
  ...
  RESIDENCE_PRINCIPALE: {
    TAUX: RULES.ifi.abattement_rp * 100,  // Abattement RP
    CONDITIONS: [
