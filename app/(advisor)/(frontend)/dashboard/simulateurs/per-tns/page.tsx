...
const PARAMS = {
  ...
  TAUX_BASE: PER_TNS.TAUX_BASE,
  TAUX_COMP: PER_TNS.TAUX_ADDITIONNEL,
  PLAFOND_MAX_PASS: 8,
  get PLAFOND_MIN() { return PER_TNS.PLANCHER },
  get PLAFOND_MAX() { return PER_TNS.PLAFOND_MAX },
...
const styles = `
  ...
  .badge-tns { display: inline-flex; align-items: center; gap: .5rem; background: linear-gradient(135deg, #1e40af 0%, #3b82f6 100%); color: #fff; padding: .35rem .75rem; border-radius: 2rem; font-size: .75rem; font-weight: 600; }
  ...
  .step.active .step-n, .step.done .step-n { color: var(--pri); }
  ...
  .card-tns { border-left: 4px solid var(--pri); }
  ...
  .narr { background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); padding: 1.25rem; border-radius: .75rem; border: 1px solid #bfdbfe; margin-bottom: 1rem; }
  .narr-t { font-weight: 600; color: #1e40af; margin-bottom: .5rem; }
  .narr-x { color: #1e3a8a; font-size: .9rem; line-height: 1.6; }
  ...
  .finp { padding: .6rem .75rem; border: 1px solid var(--brd); border-radius: .5rem; font-size: .95rem; transition: all .2s; width: 100%; }
  .finp:focus { outline: none; border-color: var(--pri); box-shadow: 0 0 0 3px rgba(30,64,175,.1); }
  .fh { font-size: .7rem; color: var(--txtm); }
  .iw { position: relative; }
  .iw .finp { padding-right: 3rem; }
  ...
  .compare-box.tns .compare-v { color: var(--pri); }
  ...
  .synth-card { background: linear-gradient(135deg, #eff6ff 0%, #fff 100%); border: 1px solid #bfdbfe; border-radius: 1rem; padding: 1.25rem; }
  ...
  .mode-opt h5 { font-size: 1rem; font-weight: 600; margin-bottom: .5rem; }
  ...
  .tbl .tns { color: var(--pri); font-weight: 600; }
  .tbl .suc { color: var(--suc); font-weight: 600; }
  .tbl .err { color: var(--err); }
  .plafond-detail { background: #fafbfc; border: 1px solid var(--brd); border-radius: .75rem; padding: 1rem; margin: 1rem 0; }
  .plafond-detail h5 { font-size: .85rem; font-weight: 600; margin-bottom: .75rem; color: var(--pri); }
  .plafond-row { display: flex; justify-content: space-between; padding: .5rem 0; border-bottom: 1px dashed var(--brd); font-size: .85rem; }
  .plafond-row:last-child { border-bottom: none; font-weight: 600; color: var(--pri); }
  @media (max-width: 768px) { .steps { flex-direction: column; } .compare { grid-template-columns: 1fr; } .compare-op { display: none; } .sortie-opts, .mode-opts { grid-template-columns: 1fr; } }
`

// =============================================================================
// FONCTIONS DE CALCUL - SPÉCIFIQUES TNS
// =============================================================================
const getTMI = (r: number) => { for (const t of PARAMS.BAREME_IR) { if (r <= t.max) return t } return PARAMS.BAREME_IR[4] }

const getPlafondTNS = (benefice: number, plRep: number) => {
  const { PASS, TAUX_BASE, TAUX_COMP, PLAFOND_MAX_PASS } = PARAMS
  const plafondMin = PASS * TAUX_BASE
  const benefPlaf = Math.min(benefice, PASS * PLAFOND_MAX_PASS)
  const plBase = benefPlaf * TAUX_BASE
  let plComp = 0
  if (benefice > PASS) plComp = Math.max(0, Math.min(benefice, PASS * PLAFOND_MAX_PASS) - PASS) * TAUX_COMP
  const plTotal = Math.max(plafondMin, plBase + plComp)
  return { plafondMin, plBase: Math.round(plBase), plComp: Math.round(plComp), plTotal: Math.round(plTotal), plAvecRep: Math.round(plTotal + plRep) }
}
...
const projPER = (cap: number, mens: number, dur: number, rend: number, tmi: number) => {
  ...
  for (let i = 0; i <= dur; i++) { if (i > 0) { const va = mens * 12; tv += va; te += va * tmi; c += va; c *= 1 + rend }; proj.push({ a: i, c: Math.round(c), v: Math.round(tv) }) }
...
const projAV = (eco: number, dur: number, rend: number) => {
  ...
  for (let i = 0; i <= dur; i++) { if (i > 0) { c += eco; tv += eco; c *= 1 + rend }; proj.push({ a: i, c: Math.round(c), v: Math.round(tv) }) }
...
const calcSortie = (cap: number, pv: number, vers: number, type: string, tmiR: number, age: number) => {
  ...
  if (type === 'capital') { const impV = vers * tmiR, impPV = pv * PARAMS.PFU, tot = impV + impPV; return { type: 'capital', brut: cap, impV, impPV, tot, net: cap - tot } }
  if (type === 'rente') { const rB = cap * 0.04, rImp = rB * partImp, imp = rImp * tmiR + rB * PARAMS.PS_RENTE, rN = rB - imp; return { type: 'rente', rBrut: rB, rNet: rN, rNetM: rN / 12, part: partImp * 100 } }
...
export default function SimulateurPERTNSPage() {
  ...
  const [res1, setRes1] = useState<{ benefice: number; tmi: number; tmiL: string; pl: number; plDetail: ReturnType<typeof getPlafondTNS>; eco: number; modeVers: string; versAnnuel: number } | null>(null)
  ...
  const c1 = useMemo(() => {
    const tmi = getTMI(form.benefice)
    const pl = getPlafondTNS(form.benefice, form.usePl ? form.plRep : 0)
    const versAnnuel = form.modeVers === 'ponctuel' ? form.versPonctuel : form.versMensuel * 12 + form.versInitial
    const imp = calcImpact(versAnnuel, pl.plAvecRep, tmi.taux)
    return { benefice: form.benefice, tmi, pl, imp, versAnnuel, modeVers: form.modeVers }
  }, [form])

  const c2 = useMemo(() => {
    if (!res1) return null
    const dur = proj.ageR - proj.age; if (dur <= 0) return null
    const rend = PARAMS.RENDEMENT_PER[proj.profil as keyof typeof PARAMS.RENDEMENT_PER]
    const capInit = res1.modeVers === 'ponctuel' ? form.versPonctuel + proj.cap : proj.cap + form.versInitial
  ...
  const c3 = useMemo(() => {
    ...
    return projAV(res1.eco, proj.ageR - proj.age, PARAMS.RENDEMENT_AV[av.profil as keyof typeof PARAMS.RENDEMENT_AV])
  }, [av, res1, res2, proj])

  const c4 = useMemo(() => res2 ? calcSortie(res2.cap, res2.pv, res2.vers, sortie.type, sortie.tmiR, proj.ageR) : null, [sortie, res2, proj.ageR])

  const v1 = () => { setRes1({ benefice: c1.benefice, tmi: c1.tmi.taux, tmiL: c1.tmi.label, pl: c1.pl.plAvecRep, plDetail: c1.pl, eco: c1.imp.eco, modeVers: c1.modeVers, versAnnuel: c1.versAnnuel }); setStep(2) }
  ...
  const drawChart = (id: string, data: { a: number; c: number; v: number }[], col: string) => {
    ...
    if (typeof window !== 'undefined' && (window as any).Plotly && data?.length) {
      ...
      ], { margin: { t: 10, r: 20, b: 40, l: 60 }, xaxis: { title: 'Années', gridcolor: '#f1f5f9' }, yaxis: { title: '€', tickformat: ',.0f', gridcolor: '#f1f5f9' }, legend: { orientation: 'h', y: -0.15 }, paper_bgcolor: 'transparent', plot_bgcolor: 'transparent', hovermode: 'x unified' }, { responsive: true, displayModeBar: false })
    }
  }
      ...
      <main className="pw">
        <div className="ph">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginTop: '.5rem' }}>
            <h1>Simulateur PER TNS</h1>
            <span className="badge-tns">Article 154 bis</span>
          </div>
          <p>Plafond Madelin étendu (max {eur(PARAMS.PLAFOND_MAX)}), projection et fiscalité à la sortie</p>
        </div>

        <div className="steps">
          {[{ n: 1, t: 'Impact Fiscal', d: 'Plafond TNS', r: res1 }, { n: 2, t: 'Projection', d: 'Capital', r: res2, l: !res1 }, { n: 3, t: 'Remploi AV', d: 'Bonus', r: res3, l: !res2 }, { n: 4, t: 'Sortie PER', d: 'Fiscalité', r: res4, l: !res2 }, { n: 5, t: 'Synthèse', d: 'Bilan', l: !res4 }].map(s => (
            <div key={s.n} className={`step ${step === s.n ? 'active' : ''} ${s.r ? 'done' : ''} ${s.l ? 'locked' : ''}`} onClick={() => !s.l && setStep(s.n)}>
              <div className="step-n">{s.r ? '✓' : s.n}</div><div className="step-t">{s.t}</div><div className="step-d">{s.d}</div>
            </div>
          ))}
        </div>
          ...
          <div className="card card-tns">
            ...
            <div className="fg">
              ...
              {form.modeVers === 'periodique' ? (<>
                <div className="fgrp"><label className="flbl">Versement mensuel</label><div className="iw"><input type="number" className="finp" value={form.versMensuel} onChange={e => setForm({ ...form, versMensuel: +e.target.value })} /><span className="sfx">€/mois</span></div><span className="fh">Soit {eur(form.versMensuel * 12)}/an</span></div>
                <div className="fgrp"><label className="flbl">Versement initial (optionnel)</label><div className="iw"><input type="number" className="finp" value={form.versInitial} onChange={e => setForm({ ...form, versInitial: +e.target.value })} /><span className="sfx">€</span></div><span className="fh">Apport unique au démarrage</span></div>
              </>) : (
                <div className="fgrp"><label className="flbl">Versement ponctuel</label><div className="iw"><input type="number" className="finp" value={form.versPonctuel} onChange={e => setForm({ ...form, versPonctuel: +e.target.value })} /><span className="sfx">€</span></div><span className="fh">Versement unique cette année</span></div>
              )}
              <div className="fgrp"><label className="flbl"><input type="checkbox" checked={form.usePl} onChange={e => setForm({ ...form, usePl: e.target.checked })} /> Plafonds non utilisés (3 ans)</label>{form.usePl && <div className="iw"><input type="number" className="finp" value={form.plRep} onChange={e => setForm({ ...form, plRep: +e.target.value })} /><span className="sfx">€</span></div>}</div>
            </div>
            {form.benefice > 0 && c1.versAnnuel > 0 && (<>
              <div className="plafond-detail">
                ...
                <div className="plafond-row"><span>Plafond total disponible</span><span>{eur(c1.pl.plAvecRep)}</span></div>
              </div>
              <div className="kpis">
                <div className="kpi"><div className="kpi-v">{eur(c1.benefice)}</div><div className="kpi-l">Bénéfice</div></div>
                <div className="kpi"><div className="kpi-v">{c1.tmi.label}</div><div className="kpi-l">TMI</div></div>
                <div className="kpi"><div className="kpi-v">{eur(c1.pl.plAvecRep)}</div><div className="kpi-l">Plafond TNS</div></div>
        ...
        {step === 2 && res1 && (
          <div className="card card-tns">
            ...
            <div className="fg">
              ...
              <div className="fgrp"><label className="flbl">Capital PER existant</label><div className="iw"><input type="number" className="finp" value={proj.cap} onChange={e => setProj({ ...proj, cap: +e.target.value })} /><span className="sfx">€</span></div></div>
          ...
          <div className="card card-w">
            ...
            {c4 && (<>
              <table className="tbl"><tbody>
                {c4.type === 'capital' && <><tr><td>Capital brut</td><td className="num tns">{eur(c4.brut)}</td></tr><tr><td>Impôt versements (IR)</td><td className="num err">- {eur(c4.impV)}</td></tr><tr><td>Impôt plus-values (PFU)</td><td className="num err">- {eur(c4.impPV)}</td></tr><tr><td><strong>Net perçu</strong></td><td className="num suc"><strong>{eur(c4.net)}</strong></td></tr></>}
