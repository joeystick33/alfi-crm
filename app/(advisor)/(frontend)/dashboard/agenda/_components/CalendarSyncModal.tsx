...
export function CalendarSyncModal({
        ...
        const mappedProviders: Provider[] = [
          {
            ...
            connected: data.providers.find((p: { id: string }) => p.id === "google")?.connected || false,
            configured: data.providers.find((p: { id: string }) => p.id === "google")?.configured || false,
            lastSync: data.providers.find((p: { id: string }) => p.id === "google")?.lastSync,
            status: data.providers.find((p: { id: string }) => p.id === "google")?.status,
          ...
          },
          {
            ...
            description: "Synchronisez avec Outlook Calendar",
  ...
  const fetchCredentialsStatus = async () => {
    try {
      const [googleRes, microsoftRes] = await Promise.all([
        fetch("/api/settings/calendar-credentials"),
  ...
  const handleDisconnect = async (providerId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        toast({
          title: "Déconnecté",
          description: "Votre calendrier a été déconnecté.",
        });
        fetchSyncStatus();
        onSyncComplete?.();
      } else {
  ...
  const handleSaveGoogleCredentials = async () => {
    ...
    try {
      const res = await fetch("/api/settings/calendar-credentials", {
        method: "POST",
      ...
      toast({
        title: "Erreur",
        description: "Impossible d'enregistrer les identifiants.",
        variant: "destructive",
      });
    } finally {
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Une fois connecté, vos événements seront synchronisés automatiquement
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
                ...
                <p className="text-xs text-slate-600 mb-4">
                  Pour activer la synchronisation Google Calendar, créez un projet sur
                  Google Cloud Console et configurez OAuth 2.0.
                </p>
