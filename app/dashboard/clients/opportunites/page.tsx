...
export default function OpportunitesDetecteesPage() {
  ...
  const resumeStats = useMemo(() => {
    if (!stats) {
      ...
      const totalPotential = opportunities.reduce((sum, o) => sum + (o.estimatedValue || o.potentialGain || 0), 0)
      const averageScore = opportunities.length > 0
        ? opportunities.reduce((sum, o) => sum + (o.score || 0), 0) / opportunities.length
        : 0
      const highPriority = opportunities.filter((o) => o.priority === 'HIGH' || o.priority === 'URGENT').length
