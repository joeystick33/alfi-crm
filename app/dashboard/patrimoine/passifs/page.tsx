...
export default function PassifsPage() {
  ...
  const totalPassifs = filteredPassifs.reduce((sum, passif) => sum + (passif.montantRestant || 0), 0);
  const totalMensualites = filteredPassifs.reduce((sum, passif) => sum + (passif.mensualite || 0), 0);
  const avgTaux = filteredPassifs.length > 0
    ? filteredPassifs.reduce((sum, passif) => sum + (passif.tauxInteret || 0), 0) / filteredPassifs.length
    : 0;
