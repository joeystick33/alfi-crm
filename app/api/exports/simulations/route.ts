...
/**
 * GET /api/exports/simulations?clientId=xxx
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Exporter les simulations
    const simulations = await exportSimulations(clientId);
    ...
    if (format === 'csv') {
      const csv = toCSV(simulations);
      const filename = generateFilename(
        `simulations_${client.firstName}_${client.lastName}`,
        '',
        'csv'
      );
    ...
    console.error('Erreur export simulations:', error);
    return createErrorResponse(
      error.message || 'Erreur lors de l\'export des simulations',
      500
    );
  }
}
