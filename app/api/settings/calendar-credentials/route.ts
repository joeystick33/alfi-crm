...
export async function POST(req: NextRequest) {
    try {
        ...
        try {
            ...
            // Gestion cabinet introuvable ou autre erreur Prisma
            if (dbError?.code === "P2025") {
                return NextResponse.json({ error: "Cabinet introuvable pour mise à jour" }, { status: 404 });
            }
            console.error("[Calendar Credentials DB Error]:", dbError);
            throw dbError;
        }
        ...
        console.error("[Calendar Credentials POST Error]:", error);
...
export async function GET(req: NextRequest) {
    try {
        ...
        // SuperAdmin peut cibler un cabinet via ?cabinetId=
        ...
        const message = error?.message || "Erreur serveur";
        console.error("[Calendar Credentials GET Error]:", error);
        return NextResponse.json({ error: message, details: String(error?.code ?? "") }, { status: 500 });
    }
}
