import { NextRequest, NextResponse } from "next/server";
import { requireAuth } from "@/app/_common/lib/auth-helpers";
import { KYCService } from "@/app/_common/lib/services/kyc-service";
import { KYCDocStatus, KYCDocumentType } from "@prisma/client";

export async function GET(req: NextRequest) {
    try {
        const { user, cabinetId, isSuperAdmin } = await requireAuth(req);

        if (!cabinetId) {
            return NextResponse.json({ error: "Cabinet non trouvé" }, { status: 400 });
        }

        const { searchParams } = new URL(req.url);
        const filters = {
            clientId: searchParams.get("clientId") || undefined,
            status: (searchParams.get("status") as KYCDocStatus) || undefined,
            type: (searchParams.get("type") as KYCDocumentType) || undefined,
        };

        const kycService = new KYCService(cabinetId, user.id, isSuperAdmin);
        const documents = await kycService.listKYCDocuments(filters);

        return NextResponse.json({ data: documents });
    } catch (error: any) {
        if (error?.message === "Unauthorized") {
            return NextResponse.json({ error: "Non authentifié" }, { status: 401 });
        }
        return NextResponse.json({ error: error.message }, { status: 500 });
    }
}

export async function POST(req: NextRequest) {
    try {
        const { user, cabinetId, isSuperAdmin } = await requireAuth(req);

        if (!cabinetId) {
            return NextResponse.json({ error: "Cabinet non trouvé" }, { status: 400 });
        }

        const payload = await req.json();
        const kycService = new KYCService(cabinetId, user.id, isSuperAdmin);

        const document = await kycService.addKYCDocument({
            ...payload,
            cabinetId, // Ensure cabinetId is set from auth
        });

        return NextResponse.json({ data: document });
    } catch (error: any) {
        if (error?.message === "Unauthorized") {
            return NextResponse.json({ error: "Non authentifié" }, { status: 401 });
        }
        return NextResponse.json({ error: error.message }, { status: 500 });
    }
}
