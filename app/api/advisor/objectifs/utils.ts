...
export function normalizeObjectifCreatePayload(body: unknown): CreateObjectifPayload {
  ...
  const targetAmount = ensureNumber((body as any).targetAmount, 'targetAmount', true)!
  const currentAmount = ensureNumber((body as any).currentAmount, 'currentAmount')
  const monthlyContribution = ensureNumber((body as any).monthlyContribution, 'monthlyContribution')
  
  const targetDate = ensureDate((body as any).targetDate, 'targetDate', true)!
...
export function normalizeObjectifUpdatePayload(body: unknown): UpdateObjectifPayload {
  ...
  const targetAmount = ensureNumber((body as any).targetAmount, 'targetAmount')
