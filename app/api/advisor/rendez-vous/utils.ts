...
export function normalizeRendezVousCreatePayload(body: unknown): CreateRendezVousPayload {
  ...
  const startDate = ensureDate((body as any).startDate, 'startDate', true)!
  const endDate = ensureDate((body as any).endDate, 'endDate', true)!
  
  const location = ensureString((body as any).location, 'location')
  const meetingUrl = ensureString((body as any).meetingUrl, 'meetingUrl')
  const isVirtual = ensureBoolean((body as any).isVirtual, 'isVirtual')
...
export function normalizeRendezVousUpdatePayload(body: unknown): UpdateRendezVousPayload {
  ...
  // Validate that virtual meetings have a meeting URL
  if (update.isVirtual && !update.meetingUrl && !(body as any).meetingUrl) {
    throw new Error('meetingUrl is required for virtual meetings')
  }
