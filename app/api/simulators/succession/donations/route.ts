...
export async function POST(request: NextRequest) {
  try {
    ...
    const { donorAge, totalWealth, targetAge, beneficiaries } = body;

    // Validate input
    if (!donorAge || donorAge <= 0 || donorAge >= 100) {
      return NextResponse.json(
        { error: 'Âge du donateur invalide' },
        { status: 400 }
      );
    }
    ...
    if (!totalWealth || totalWealth <= 0) {
      return NextResponse.json(
        { error: 'Patrimoine invalide' },
        { status: 400 }
      );
    }

    if (!targetAge || targetAge <= donorAge) {
      return NextResponse.json(
        { error: 'Âge cible invalide' },
        { status: 400 }
      );
    }
    ...
    if (!beneficiaries || !Array.isArray(beneficiaries) || beneficiaries.length === 0) {
      return NextResponse.json(
        { error: 'Les bénéficiaires sont requis' },
        { status: 400 }
      );
    }
    ...
    const totalDonationTax = beneficiaryBreakdown.reduce((sum, b) => sum + b.totalTax, 0);
