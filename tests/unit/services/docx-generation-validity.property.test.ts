/**
 * Property-Based Tests - DOCX Generation Validity
 * 
 * **Feature: crm-audit-fonctionnel, Property 4: DOCX Generation Validity**
 * **Validates: Requirements 4.1, 4.2, 4.3, 4.4, 4.5**
 * 
 * Tests that DOCX generation produces valid files:
 * - 4.1: Uses real docx library to produce .docx files
 * - 4.2: Preserves all formatting, headers, footers, and styles
 * - 4.3: Includes editable placeholders for custom content
 * - 4.4: Applies cabinet branding (logo, colors, fonts)
 * - 4.5: Saves file to storage and returns real download URL
 */
 ...
 */
const missionDataArb: fc.Arbitrary<MissionData> = fc.record({
 ...
 */
const patrimoineDataArb: fc.Arbitrary<PatrimoineData | undefined> = fc.option(fc.record({
  actifs: fc.array(
    fc.record({
 ...
 */
const revenusDataArb: fc.Arbitrary<RevenusData | undefined> = fc.option(fc.record({
 ...
 */
const chargesDataArb: fc.Arbitrary<ChargesData | undefined> = fc.option(fc.record({
...
describe('DOCX Generation Validity - Property-Based Tests', () => {
  /**
   * **Feature: crm-audit-fonctionnel, Property 4: DOCX Generation Validity**
   * **Validates: Requirements 4.1-4.5**
   */
  describe('Property 4: DOCX Generation Validity', () => {
    /**
     * Requirement 4.1: DER DOCX generation produces valid DOCX
     */
    it('DER DOCX generation produces valid DOCX with size > 0', async () => {
      await fc.assert(
        fc.asyncProperty(
          clientDataArb,
          cabinetDataArb,
          ...
          async (client, cabinet, advisor) => {
            ...
            // Should be a valid DOCX (ZIP archive starting with PK)
    ...
    /**
     * Requirement 4.1: Déclaration d'Adéquation DOCX generation produces valid DOCX
     */
    it('Déclaration d\'Adéquation DOCX generation produces valid DOCX with size > 0', async () => {
      await fc.assert(
        fc.asyncProperty(
          clientDataArb,
          cabinetDataArb,
          ...
          async (client, cabinet, advisor, product, justification, warnings) => {
            const result = await generateDeclarationAdequationDOCX(
              client,
              cabinet,
              advisor,
              product,
    ...
    /**
     * Requirement 4.1: Bulletin d'Opération DOCX generation produces valid DOCX
     */
    it('Bulletin d\'Opération DOCX generation produces valid DOCX with size > 0', async () => {
      await fc.assert(
        fc.asyncProperty(
          clientDataArb,
          cabinetDataArb,
          ...
          async (client, cabinet, advisor, operation, checklist) => {
            const result = await generateBulletinOperationDOCX(
              client,
              cabinet,
              advisor,
              operation,
    ...
    /**
     * Requirement 4.1: Lettre de Mission DOCX generation produces valid DOCX
     */
    it('Lettre de Mission DOCX generation produces valid DOCX with size > 0', async () => {
      await fc.assert(
        fc.asyncProperty(
          clientDataArb,
          cabinetDataArb,
          ...
          async (client, cabinet, advisor, mission) => {
            const result = await generateLettreMissionDOCX(
              client,
              cabinet,
              advisor,
              mission
    ...
    /**
     * Requirement 4.1: Recueil d'Informations DOCX generation produces valid DOCX
     */
    it('Recueil d\'Informations DOCX generation produces valid DOCX with size > 0', async () => {
      await fc.assert(
        fc.asyncProperty(
          clientDataArb,
          cabinetDataArb,
          ...
          async (client, cabinet, advisor, patrimoine, revenus, charges) => {
            const result = await generateRecueilInformationsDOCX(
              client,
              cabinet,
              advisor,
              patrimoine,
    ...
    /**
     * Requirement 4.1: All supported document types are defined
     */
    it('all supported document types are defined', () => {
      fc.assert(
        fc.property(fc.constantFrom(...SUPPORTED_DOCX_DOCUMENT_TYPES), (docType) => {
          expect(SUPPORTED_DOCX_DOCUMENT_TYPES).toContain(docType)
    ...
    /**
     * Requirement 4.5: DOCX generation without storage does not return fileUrl
     */
    it('DOCX generation without storage does not return fileUrl', async () => {
      await fc.assert(
        fc.asyncProperty(
          clientDataArb,
          cabinetDataArb,
          advisorDataArb,
          async (client, cabinet, advisor) => {
            const result = await generateDERDOCX(client, cabinet, advisor, { uploadToStorage: false })
    ...
    /**
     * DOCX generation produces valid files for same inputs (both succeed)
     * Note: We don't check file size equality because DOCX files (ZIP archives) 
     * may include timestamps or metadata that varies between runs
     */
    it('DOCX generation produces valid files for same inputs', async () => {
      await fc.assert(
        fc.asyncProperty(
          clientDataArb,
          cabinetDataArb,
          advisorDataArb,
          async (client, cabinet, advisor) => {
            const result1 = await generateDERDOCX(client, cabinet, advisor)
            const result2 = await generateDERDOCX(client, cabinet, advisor)
            
            // Both should succeed
            expect(result1.success).toBe(true)
            expect(result2.success).toBe(true)
            
            // Both should have valid file sizes (> 0)
            expect(result1.fileSize).toBeGreaterThan(0)
            expect(result2.fileSize).toBeGreaterThan(0)
            
            // Both should be valid DOCX
            expect(isValidDOCX(result1.fileBuffer!)).toBe(true)
            expect(isValidDOCX(result2.fileBuffer!)).toBe(true)
          }
        ),
        { numRuns: 20 } // Reduced runs since each iteration generates 2 DOCX files
      )
    }, 60000) // 60 second timeout

    /**
     * isValidDOCX correctly identifies valid and invalid DOCX files
     */
    it('isValidDOCX correctly validates DOCX headers (ZIP signature)', () => {
      fc.assert(
        fc.property(fc.uint8Array({ minLength: 10, maxLength: 100 }), (randomBytes) => {
          const buffer = Buffer.from(randomBytes)
          
          // Create a valid ZIP header (DOCX files are ZIP archives)
          const validDocxBuffer = Buffer.concat([Buffer.from([0x50, 0x4B, 0x03, 0x04]), buffer])
          expect(isValidDOCX(validDocxBuffer)).toBe(true)
          
          // Random bytes should not be valid DOCX (unless they happen to start with PK)
          const startsWithZipHeader = buffer[0] === 0x50 && buffer[1] === 0x4B
          expect(isValidDOCX(buffer)).toBe(startsWithZipHeader)
        }),
        { numRuns: 100 }
      )
    })
    ...
    /**
     * Empty or small buffers are not valid DOCX files
     */
    it('empty or small buffers are not valid DOCX files', () => {
      fc.assert(
        fc.property(fc.uint8Array({ minLength: 0, maxLength: 3 }), (smallBytes) => {
