  ...
  return (
    <Card className="border-slate-200" role="region" aria-label="Widget des opportunités commerciales">
      <CardHeader>
        ...
        {/* Pipeline Summary */}
        <div className="mt-3 p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200" role="status" aria-label={`Pipeline total: ${formatCurrency(totalPipelineValue)}, ${stats.active} opportunités actives, score moyen ${stats.avgScore}`}>
          <div className="flex items-center justify-between">
            <div>
            ...
            <div className="text-right">
              ...
              <div className="flex items-center gap-1 text-xs text-green-700">
                ...
                <span aria-label={`Score moyen: ${stats.avgScore}`}>Score moy: {stats.avgScore}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3" role="list" aria-labelledby="opportunities-widget-title">
            {opportunities.map((opp) => {
              const clientName = opp.particulierId?.firstName && opp.particulierId?.lastName 
              ...
              const oppAriaLabel = `Opportunité ${getTypeLabel(opp.type)} pour ${clientName}, valeur estimée ${formatCurrency(opp.estimatedValue)}, statut ${statusConfig.label}, progression ${progress}%${requiresAction ? ', action requise' : ''}`;

              return (
                <div
                  key={opp._id}
                  className={`p-3 rounded-lg border transition-all cursor-pointer hover:shadow-md ${priorityConfig.color}`}
                  onClick={() => router.push(`/dashboard/opportunities/${opp._id}`)}
                  role="listitem"
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      router.push(`/dashboard/opportunities/${opp._id}`);
                    }
                  }}
                >
                  {/* Header */}
