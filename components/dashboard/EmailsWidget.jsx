 ...
 import { Mail, Paperclip, Plus, Search, Star, Trash2, Reply } from 'lucide-react'
 ...
 export default function EmailsWidget() {
         ...
         {list.map((email) => {
           ...
           return (
             <div
               key={email.id}
               className={cn(
                 'group flex items-start gap-3 p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-all duration-200 cursor-pointer relative',
                 unread ? 'bg-blue-50/30 dark:bg-blue-900/10' : ''
               )}
             >
               ...
               <div className="flex-1 min-w-0 space-y-0.5">
                 <div className="flex items-center justify-between">
                   ...
                   <span
                     className={cn(
                       ...
                       unread ? 'text-blue-600 dark:text-blue-400 font-medium' : 'text-slate-400'
                     )}
                   >
                     {time}
                   </span>
                 ...
                 <div className="flex items-center gap-2">
                   <span
                     className={cn(
                       ...
                       unread ? 'text-slate-900 dark:text-white font-medium' : 'text-slate-600 dark:text-slate-400'
                     )}
                   >
                 ...
                 <div className="absolute right-4 bottom-3 hidden group-hover:flex items-center gap-1 bg-white/90 dark:bg-slate-800/90 p-1 rounded-md shadow-sm backdrop-blur-sm border border-slate-200 dark:border-slate-700">
   ...
   return (
     <Card className="h-full shadow-sm border-slate-200 dark:border-slate-800 bg-white dark:bg-gray-900/50 backdrop-blur-sm flex flex-col overflow-hidden">
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between gap-4 bg-slate-50/50 dark:bg-slate-900/50">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-lg bg-primary/10">
            <Mail className="h-5 w-5 text-primary" />
          </div>
          <h3 className="font-semibold text-slate-900 dark:text-white">Messagerie</h3>
        </div>
        ...
        <div className="flex items-center gap-2 flex-1 justify-end">
          <div className="relative w-full max-w-[200px] hidden sm:block">
            ...
            <Input 
              ...
              className="h-8 pl-8 bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-xs"
            />
          </div>
      ...
      <Tabs defaultValue="inbox" value={tab} onValueChange={setTab} className="flex-1 flex flex-col">
        <div className="px-4 pt-2">
          <TabsList className="grid w-full grid-cols-2 h-9 bg-slate-100 dark:bg-slate-800 p-1">
