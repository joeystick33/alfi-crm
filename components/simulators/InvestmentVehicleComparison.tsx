...
export function InvestmentVehicleComparison() {
  ...
  const compareVehicles = async () => {
    ...
    try {
      const response = await fetch('/api/simulators/tax/investment-vehicles', {
        method: 'POST',
      ...
      setError(err.message || 'Erreur lors de la comparaison des véhicules');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const comparisonChartData = result?.comparisons ? result.comparisons.map((v: any) => ({
  ...
  const taxRateChartData = result?.comparisons ? result.comparisons.map((v: any) => ({
    name: v.name.split(' ')[0],
