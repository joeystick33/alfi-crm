...
async function testServiceInstantiation() {
  ...
  try {
    ...
    const { DocumentService } = await import('@/app/_common/lib/services/document-service')
    const documentService = new DocumentService(cabinet.id, user.id, false)
    logTest('/api/documents', 'SERVICE', true, 'DocumentService instantiated successfully')

    // Test ProjetService
    const { ProjetService } = await import('@/app/_common/lib/services/projet-service')
    ...
    const { OpportuniteService } = await import('@/app/_common/lib/services/opportunite-service')
    ...
    // Test ClientService
    const { ClientService } = await import('@/app/_common/lib/services/client-service')
    ...
    const { ObjectifService } = await import('@/app/_common/lib/services/objectif-service')
    const objectifService = new ObjectifService(cabinet.id, user.id, false)
    logTest('/api/objectifs', 'SERVICE', true, 'ObjectifService instantiated successfully')

    // Test SimulationService
    const { SimulationService } = await import('@/app/_common/lib/services/simulation-service')
    ...
    const { NotificationService } = await import('@/app/_common/lib/services/notification-service')
    const notificationService = new NotificationService(cabinet.id, user.id, user.role, false)
    logTest('/api/advisor/notifications', 'SERVICE', true, 'NotificationService instantiated successfully')

    // Test RendezVousService
    const { RendezVousService } = await import('@/app/_common/lib/services/rendez-vous-service')
    const rendezVousService = new RendezVousService(cabinet.id, user.id, false)
...
async function testValidationUtilities() {
  ...
  try {
    // Test document utils
    const documentUtils = await import('@/app/(advisor)/(backend)/api/advisor/documents/utils')
    logTest('/api/documents/utils', 'UTILS', true, 'Document utils imported successfully', {
      exports: Object.keys(documentUtils)
    })

    // Test projet utils
    const projetUtils = await import('@/app/(advisor)/(backend)/api/advisor/projets/utils')
    ...
    const opportuniteUtils = await import('@/app/(advisor)/(backend)/api/advisor/opportunites/utils')
    logTest('/api/opportunites/utils', 'UTILS', true, 'Opportunite utils imported successfully', {
      exports: Object.keys(opportuniteUtils)
    })

    // Test client utils
    const clientUtils = await import('@/app/(advisor)/(backend)/api/advisor/clients/utils')
    logTest('/api/clients/utils', 'UTILS', true, 'Client utils imported successfully', {
      exports: Object.keys(clientUtils)
    })

    // Test objectif utils
    const objectifUtils = await import('@/app/(advisor)/(backend)/api/advisor/objectifs/utils')
    ...
    const simulationUtils = await import('@/app/(advisor)/(backend)/api/advisor/simulations/utils')
    ...
    const notificationUtils = await import('@/app/(advisor)/(backend)/api/advisor/notifications/utils')
    logTest('/api/advisor/notifications/utils', 'UTILS', true, 'Notification utils imported successfully', {
      exports: Object.keys(notificationUtils)
    })

    // Test rendez-vous utils
    const rendezVousUtils = await import('@/app/(advisor)/(backend)/api/advisor/rendez-vous/utils')
